...
与`TTTBoard`类的`getBoardStr()`方法一样，`MiniBoard`的`getBoardStr()`方法创建了一个井字游戏棋盘的多行字符串，在传递给`print()`函数时显示。但是这个字符串要小得多，放弃了 X 和 O 标记之间的线，使用点号来表示空格。
