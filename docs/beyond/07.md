...
`format()` string 方法添加了*格式规范迷你语言*([`docs.python.org/3/library/string.html#formatspec`](https://docs.python.org/3/library/string.html#formatspec))，这涉及到以类似于`%s`转换说明符的方式使用`{}`大括号对。然而，这种方法有些复杂，会产生不可读的代码，所以我不鼓励使用它。
