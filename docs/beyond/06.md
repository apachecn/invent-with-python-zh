...
导入`logging`模块并设置其基本配置后，您可以调用`logging.debug()`将信息写入文本文件，而不是使用`print()`将其显示在屏幕上。与打印调试不同，调用`logging.debug()`可以很明显地看出什么输出是调试信息，什么输出是程序正常运行的结果。你可以在(Automate the Boring Stuff with Python)第 11 章找到更多关于调试的信息，你可以在[https://autbor.com/2e/c11/](https://autbor.com/2e/c11/)在线阅读。
