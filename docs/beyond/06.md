...
这是一个很好的解决方案，但更好的方案是用常量替换这些“神奇”的数字。*常量*是变量，其名称以大写字母书写，表示其值在初始赋值后不应改变。通常，常量在源代码文件的顶部被定义为全局变量:
...
这些理解是简洁的，可以使你的代码更具可读性。但是请注意，综合基于一个 可迭代对象(在本例中是由`range(100)`调用返回的`range`对象)生成一个列表、集合或字典。列表、集合和字典都是可迭代对象，这意味着您可以将列表嵌套在列表中，如下例所示:
