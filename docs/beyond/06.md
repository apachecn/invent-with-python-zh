...
> 原文:[http://inventwithpython.com/beyond/chapter5.html](http://inventwithpython.com/beyond/chapter5.html)

![](../Images/74e8cef2461199147c9866361cd78f1c.png)导致程序崩溃的代码显然是错误的，但是崩溃并不是程序中问题的唯一指示器。其他迹象可能表明存在更微妙的错误或不可读的代码。就像气体的味道可以指示气体泄漏或者烟雾的味道可以指示火灾一样，*代码味道*是指示潜在错误的源代码模式。代码气味并不一定意味着存在问题，但它确实意味着您应该调查您的程序。

本章列出了几种常见的代码味道。预防一个 bug 比以后遇到、理解并修复一个 bug 花费的时间和精力要少得多。每个程序员都有这样的故事:花了几个小时调试，却发现修复只需要修改一行代码。出于这个原因，即使是一点潜在的错误也应该让你停下来，提醒你再次检查你没有在制造未来的问题。

当然，代码味道不一定是问题。最终，是解决还是忽略代码气味是您自己的判断。

## 重复代码

最常见的代码味道就是*重复代码*。重复代码是您通过将一些其他代码复制并粘贴到程序中而创建的任何源代码。例如，这个短程序包含重复的代码。请注意，它三次询问用户的感受:
...
与所有代码气味一样，避免重复代码并不是一个必须始终遵循的硬性规则。一般来说，重复代码段越长，或者程序中出现的重复副本越多，就越有必要进行重复数据删除。我不介意复制粘贴一次甚至两次代码。但是，当我的程序中存在三个或四个副本时，我通常会考虑对代码进行重复数据删除。
...
重复代码是一种代码味道，因为它使您的代码更难一致地更改。如果程序中有几个重复的代码，解决方法是将代码放在一个函数或循环中，这样它只出现一次。
...
即使幻数是相同的，也应该为不同用途的幻数使用不同的常数。例如，一副扑克牌中有 52 张牌，一年有 52 周。但是如果你的程序中有这两个量，你应该做如下的事情:
...
术语*幻数*也适用于非数值。例如，您可以使用字符串值作为常量。考虑下面的程序，它要求用户输入一个方向，如果方向是北，就显示一个警告。一个`'nrth'`输入错误导致程序无法显示警告:
...
这个错误很难被发现:字符串`'nrth'`中的错别字 1 仍然是语法正确的 Python。程序不会崩溃，并且很容易忽略没有警告信息。但是如果我们使用常量并犯了同样的错误，这个错误会导致程序崩溃，因为 Python 会注意到一个`NRTH`常量并不存在:
...
由带有`NRTH`错别字 1 的代码行引发的`NameError`异常使得当您运行该程序时，该错误立即变得明显:
...
幻数是一种代码味道，因为它们没有传达它们的目的，使你的代码可读性更差，更难更新，并且容易出现不可察觉的打字错误。解决方法是使用常量变量。

## 注释掉的代码和死代码

注释掉代码使其不运行作为一种临时措施是好的。您可能希望跳过一些行来测试其他功能，将它们注释掉便于以后添加回去。但是，如果注释掉的代码仍然存在，那么它为什么被删除以及在什么情况下可能会再次需要它就完全是个谜了。考虑下面的例子:
...
这段代码提示了许多没有答案的问题:为什么`doAnotherThing()`被注释掉了？我们还会把它包括进来吗？为什么第二次调用`doAnotherThing()`没有被注释掉？最初有两个打给`doAnotherThing()`的电话，还是有一个电话在`doSomeImportantTask()`之后被转移了？我们有理由不删除被注释掉的代码吗？这些问题没有现成的答案。

*死代码*是不可达或者逻辑上永远无法运行的代码。例如，在一个函数内部但是在一个`return`语句之后的代码，在一个具有总是`False`条件的`if`语句块中的代码，或者在一个从来没有被调用的函数中的代码都是死代码。要在实践中看到这一点，请在交互式 shell 中输入以下内容:
...
`return 'The coin landed on its edge!'`行是死代码，因为`if` 和`else`块中的代码在执行到达该行之前返回。死代码具有误导性，因为阅读它的程序员认为它是程序的活动部分，而实际上它与注释掉的代码是一样的。

*存根*是这些代码味道规则的一个例外。这些是未来代码的占位符，比如尚未实现的函数或类。代替真正的代码，存根包含一个`pass`语句，它什么也不做。(也称*不操作*或*不操作*。)存在`pass`语句，因此您可以在语言语法需要一些代码的地方创建存根:
...
当这个函数被调用时，它什么也不做。相反，它指示代码最终将被添加到。
...
注释掉的代码和死代码是代码气味，因为它们会误导程序员认为代码是程序的可执行部分。相反，删除它们并使用版本控制系统，如 Git 或 Subversion，来跟踪变更。版本控制包含在第 12 章中。有了版本控制，您可以从程序中删除代码，如果需要，以后可以很容易地将代码添加回去。
...
1.  注意你程序中的一个错误。
2.  Add `print()`调用一些变量来找出它们包含的内容。
3.  重新运行程序。
4.  再加一些`print()`的通话，因为之前的没有显示足够的信息。
5.  重新运行程序。
6.  在最终找出错误之前，重复前面的两个步骤几次。
7.  重新运行程序。
8.  意识到你忘了移除一些`print()`呼叫并移除它们。

打印调试看似快速简单。但是在显示修复 bug 所需的信息之前，通常需要多次重复运行程序。解决方案是使用调试器或为程序设置日志文件。通过使用调试器，您可以一次运行一行代码并检查任何变量。使用调试器可能看起来比简单地插入一个`print()`调用要慢，但是从长远来看，它可以节省您的时间。
...
导入`logging`模块并设置其基本配置后，您可以调用`logging.debug()`将信息写入文本文件，而不是使用`print()`将其显示在屏幕上。与打印调试不同，调用`logging.debug()`可以很明显地看出什么输出是调试信息，什么输出是程序正常运行的结果。你可以在*用 Python* 自动化枯燥的东西第二版(无淀粉，2019)第 11 章找到更多关于调试的信息，你可以在[https://autbor.com/2e/c11/](https://autbor.com/2e/c11/)在线阅读。
...
这种代码味道并不适用于每一个仅仅以数字结尾的变量。例如，有一个名为`enableIPv6`的变量完全没问题，因为“6”是“IPv6”专有名称的一部分，而不是数字后缀。但是，如果您对一系列变量使用数字后缀，请考虑用一种数据结构(如列表或字典)来替换它们。
...
与其他语言相比，Python 使用一种随意的方法来组织代码，因为它的代码不需要存在于类或其他样板结构中。如果您发现创建对象只是为了进行单个函数调用，或者如果您编写的类只包含静态方法，这些代码气味表明您可能更适合编写函数。

在 Python 中，我们使用模块而不是类来将函数组合在一起。因为类无论如何都必须在一个模块中，所以将代码放在类中只会给代码增加一个不必要的组织层。第 15 章到 17 章更详细地讨论了这些面向对象的设计原则。Jack Diederich 的 PyCon 2012 演讲“停止编写类”涵盖了其他可能过度复杂 Python 代码的方式。

## 列表理解中的列表理解

列表理解是一种创建复杂列表值的简洁方法。例如，要为数字 0 到 100 创建一个数字串列表，排除所有 5 的倍数，通常需要一个`for`循环:
...
集合理解 1 用大括号代替方括号，产生一个集合值。字典理解 2 产生一个字典值，并使用冒号来分隔理解中的键和值。

这些理解是简洁的，可以使你的代码更具可读性。但是请注意，综合基于一个 iterable 对象(在本例中是由`range(100)`调用返回的`range`对象)生成一个列表、集合或字典。列表、集合和字典都是可迭代对象，这意味着您可以将理解嵌套在理解中，如下例所示:
...
理解是句法上的捷径，可以产生简洁的代码，但是不要走极端，把它们嵌套在一起。
...
有些代码气味根本不是真正的代码气味。编程充满了不太为人所知的坏建议，这些建议被断章取义，或者在它们失去效用后仍然存在。我责怪那些试图将自己的主观观点作为最佳实践的科技书籍作者。

你可能已经被告知这些实践中的一些是代码味道，但是它们大部分是好的。我称之为*代码气味神话*:它们是你可以也应该忽略的警告。让我们来看看其中的几个。
...
这个代码气味神话的支持者认为，因为函数应该总是只做一件事，而错误处理是一件事，所以我们应该把这个函数分成两个函数。他们认为，如果你使用一个`try` - `except`语句，它应该是函数中的第一个语句，并封装函数的所有代码，如下所示:
...
事实上，如果你的函数看起来像这样，你应该创建两个独立的函数，而不是用一个参数来决定运行哪一半的函数代码。但是大多数带标志参数的函数不这么做。例如，您可以为`sorted()`函数的`reverse`关键字参数传递一个布尔值来确定排序顺序。将函数分成两个名为`sorted()`和`reverseSorted()`的函数并不能改进代码(同时也增加了所需文档的数量)。因此，标志参数总是不好的想法是一个代码味道的神话。

### 误解:全局变量是不好的

函数和方法就像程序中的迷你程序:它们包含代码，包括函数返回时被遗忘的局部变量。这类似于程序终止后变量被遗忘的情况。函数是独立的:它们的代码要么正确执行，要么有错误，这取决于调用它们时传递的参数。

但是使用全局变量的函数和方法失去了一些有用的隔离。你在函数中使用的每一个全局变量实际上都变成了函数的另一个输入，就像参数一样。更多的参数意味着更多的复杂性，这反过来意味着更高的错误可能性。如果由于全局变量中的错误值而导致函数中出现 bug，那么这个错误值可能被设置在程序中的任何地方。要搜索这个坏值的可能原因，不能只分析函数中的代码或调用函数的代码行；您必须查看整个程序的代码。因此，您应该限制全局变量的使用。
...
全局变量增加了调试的工作量，以找到可能设置了导致异常的值的位置。这使得大量使用全局变量成为一个坏主意。但是认为所有的全局变量都是坏的是一个代码味道的神话。全局变量在较小的程序中或者在跟踪应用于整个程序的设置时非常有用。如果你可以避免使用全局变量，那就意味着你应该避免使用全局变量。但是“全局变量是坏的”是一种过于简单化的观点。

### 误解:评论是不必要的

差评确实比完全没有评论更糟糕。带有过时或误导信息的注释会给程序员带来更多的工作，而不是更好的理解。但是这个潜在的问题有时被用来宣告所有的评论都是不好的。这种观点认为每一个注释都应该用可读性更好的代码来代替，以至于程序根本就不应该有注释。

注释是用英语(或者程序员说的任何一种语言)写的，这允许它们在某种程度上传达变量、函数和类名所不能传达的信息。但是写简洁有效的评论是很难的。注释和代码一样，需要重写和多次迭代才能正确。我们在写完代码后马上就理解了它，所以写注释看起来像是无意义的额外工作。结果，程序员们倾向于接受“注释是不必要的”这一观点。

更常见的经验是程序的注释太少或没有，而不是太多或误导性的注释。拒绝评论就像说，“乘坐客机飞越大西洋只有 99.999991%的安全，所以我要游过去。”

第 10 章有更多关于如何写有效评论的信息。

## 摘要

代码气味表明可能有更好的方式来编写代码。他们不一定要求改变，但他们应该让你再看一眼。最常见的代码味道是重复代码，这可能意味着有机会将代码放在函数或循环中。这确保了未来的代码更改只需要在一个地方进行。其他代码气味包括幻数，幻数是代码中无法解释的值，可以用具有描述性名称的常数来替换。类似地，注释掉的代码和死代码永远不会被计算机运行，可能会误导后来阅读程序代码的程序员。如果您以后需要将它们添加回您的程序中，最好将它们移除，并依靠像 Git 这样的源代码控制系统。

打印调试使用`print()`调用来显示调试信息。尽管这种调试方法很容易，但从长远来看，依靠调试器和日志来诊断错误通常更快。

带有数字后缀的变量，比如`x1`、`x2`、`x3`等等，通常最好用包含列表的单个变量来替换。与 Java 等语言不同，在 Python 中，我们使用模块而不是类来将函数组合在一起。包含单个方法或仅包含静态方法的类是一种代码味道，建议您应该将代码放入模块而不是类中。尽管列表理解是一种创建列表值的简洁方法，但是嵌套的列表理解通常是不可读的。

此外，任何用空的`except`块处理的异常都是一种代码味道，你只是在消除错误，而不是处理它。一条简短、隐晦的错误消息对用户来说就像没有错误消息一样毫无用处。

伴随着这些代码味道的是代码味道神话:不再有效的编程建议，或者随着时间的推移，已经证明适得其反。这些包括在每个函数中只放一个`return`语句或`try` - `except`块，从不使用标志参数或全局变量，并且认为注释是不必要的。

当然，和所有编程建议一样，本章描述的代码味道可能适用于也可能不适用于您的项目或个人偏好。最佳实践不是一个客观的衡量标准。随着您获得更多的经验，您会对什么代码是可读的或可靠的得出不同的结论，但是本章中的建议概述了需要考虑的问题。