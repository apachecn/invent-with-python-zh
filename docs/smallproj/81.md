...
![](../Images/9d995d63aaead72cad01120081eb8f75.png)被误认为是 19 世纪密码学家 Blaise de Vigenère(其他人早些时候独立发明了它)的 Vigenère 密码在数百年内都不可能被破解。它本质上是凯撒密码，除了它使用了多部分密钥。所谓的 *Vigenère key* 就是一个单词，甚至是一串随机的字母。每个字母代表一个数字，该数字代表消息中的字母移动: *A* 代表消息中的字母移动 0， *B* 代表 1， *C* 代表 2，依此类推。
