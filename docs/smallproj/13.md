...
```py
"""Collatz Sequence, by Al Sweigart email@protected #  1
Generates numbers for the Collatz sequence, given a starting number. #  2
More info at: https://en.wikipedia.org/wiki/Collatz_conjecture #  3
This code is available at https://nostarch.com/big-book-small-python-programming #  4
Tags: tiny, beginner, math""" #  5
 #  6
import sys, time #  7
 #  8
print('''Collatz Sequence, or, the 3n + 1 Problem #  9
By Al Sweigart email@protected # 10
 # 11
The Collatz sequence is a sequence of numbers produced from a starting # 12
number n, following three rules: # 13
 # 14
1) If n is even, the next number n is n / 2. # 15
2) If n is odd, the next number n is n * 3 + 1. # 16
3) If n is 1, stop. Otherwise, repeat. # 17
 # 18
It is generally thought, but so far not mathematically proven, that # 19
every starting number eventually terminates at 1. # 20
''') # 21
 # 22
print('Enter a starting number (greater than 0) or QUIT:') # 23
response = input('> ') # 24
 # 25
if not response.isdecimal() or response == '0': # 26
    print('You must enter an integer greater than 0.') # 27
    sys.exit() # 28
 # 29
n = int(response) # 30
print(n, end='', flush=True) # 31
while n != 1: # 32
    if n % 2 == 0:  # If n is even... # 33
        n = n // 2 # 34
    else:  # Otherwise, n is odd... # 35
        n = 3 * n + 1 # 36
 # 37
    print(', ' + str(n), end='', flush=True) # 38
    time.sleep(0.1) # 39
print()  # 40
```
