...
![](../Images/9d995d63aaead72cad01120081eb8f75.png)汉诺塔是一款移动堆叠的益智游戏，有三根柱子，你可以在上面堆叠不同大小的圆盘。游戏的目标是将一个塔盘移到另一个柱子上。但是，一次只能移动一个磁盘，较大的磁盘不能放在较小的磁盘上。找出某种模式将有助于你解决这个难题。你能发现它吗？(提示:尝试将`TOTAL_DISKS`变量设置为`3`或`4`，以先解决一个更简单的版本。)
