...
PigLatin是一种文字游戏，它将英语单词转换成对拉丁语的模仿。在PigLatin中，如果一个单词以辅音开头，说话者会把这个字母去掉，放在末尾，后面跟着`ay`比如`pig`变成了`igpay`，`latin`变成了`atinlay`。否则，如果单词以元音开头，说话者只需在末尾加上`yay`即可。例如，`elephant`变成了`elephantyay`，`umbrella`变成了`umbrellayay`
