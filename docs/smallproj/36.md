...
```py
"""Hex Grid, by Al Sweigart email@protected #  1
Displays a simple tessellation of a hexagon grid. #  2
This code is available at https://nostarch.com/big-book-small-python-programming #  3
Tags: tiny, beginner, artistic""" #  4
 #  5
# Set up the constants: #  6
# (!) Try changing these values to other numbers: #  7
X_REPEAT = 19  # How many times to tessellate horizontally. #  8
Y_REPEAT = 12  # How many times to tessellate vertically. #  9
 # 10
for y in range(Y_REPEAT): # 11
    # Display the top half of the hexagon: # 12
    for x in range(X_REPEAT): # 13
        print(r'/ \_', end='') # 14
    print() # 15
 # 16
    # Display the bottom half of the hexagon: # 17
    for x in range(X_REPEAT): # 18
        print(r'\_/ ', end='') # 19
    print()  # 20
```
