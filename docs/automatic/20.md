...
*裁剪图像是指选择图像内的一个矩形区域，并删除矩形外的所有内容。对`Image`对象的`crop()`方法接受一个盒子元组，并返回一个表示裁剪图像的`Image`对象。裁剪不会原地发生——也就是说，原始的`Image`对象保持不变，而`crop()`方法返回一个新的`Image`对象。请记住，装箱的元组(在这种情况下，裁剪部分)包括左列和顶行像素，但仅向上和不包括右列和底行像素。*
