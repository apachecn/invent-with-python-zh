...
Pillow 支持大量的颜色名称，从`'aliceblue'`到`'whitesmoke'`。你可以在 https://nostarch.com/automatestuff2/的资源中找到 100 多种标准颜色名称的完整列表。
...
现在你知道了颜色和坐标在 Pillow 中是如何工作的，让我们使用 Pillow 来操作一个图像。图 19-3 是本章中所有交互式外壳示例将使用的图像。可以从 https://nostarch.com/automatestuff2/的*T3*下载。
...
对于`ImageDraw`对象，还有其他几种形状绘制方法。完整文档可在*[https://pillow . readthedocs . io/en/latest/reference/imagedraw . html](https://pillow.readthedocs.io/en/latest/reference/ImageDraw.html)*获取。
