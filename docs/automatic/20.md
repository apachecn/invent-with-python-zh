...
输入 Python。Pillow 是一个用于与图像文件交互的第三方 Python 模块。该模块有几个功能，可以轻松地裁剪、调整和编辑图像的内容。Python 能够像处理 Microsoft Paint 或 Adobe Photoshop 等软件一样处理图像，因此可以轻松地自动编辑成百上千的图像。运行`pip install --user -U pillow==6.0.0`就可以安装枕头了。附录 A 有更多关于安装模块的细节。
...
现在你知道了颜色和坐标在 Pillow 中是如何工作的，让我们使用 Pillow 来操作一个图像。图 19-3 是本章中所有交互式Shell示例将使用的图像。可以从`nostarch.com/automatestuff2`下载。
...
*裁剪图像是指选择图像内的一个矩形区域，并删除矩形外的所有内容。对`Image`对象的`crop()`方法接受一个盒子元组，并返回一个表示裁剪图像的`Image`对象。裁剪不会就地发生——也就是说，原始的`Image`对象保持不变，而`crop()`方法返回一个新的`Image`对象。请记住，装箱的元组(在这种情况下，裁剪部分)包括左列和顶行像素，但仅向上和不包括右列和底行像素。*
...
这为裁剪后的图像创建了一个新的`Image`对象，将该对象存储在`croppedIm`中，然后在`croppedIm`上调用`save()`将裁剪后的图像保存在【cropped.png】的中。新文件`cropped.png`将从原始图像中创建，如图 19-4 中的所示。
...
第一个调用将图像旋转 6 度并保存到 rotate6.png 的*(见图 19-8 左图)。第二个调用在`expand`设置为`True`的情况下将图像旋转 6 度，并保存到 `rotate6_expanded.png` (见图 19-8 右图)。*
