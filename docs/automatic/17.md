...
在[第 15 章](#calibre_link-477)，你学习了如何从 PDF 和 Word 文档中提取文本。这些文件是二进制格式的，需要特殊的 Python 模块来访问它们的数据。另一方面，CSV 和 JSON 文件只是纯文本文件。您可以在文本编辑器(如 Mu)中查看它们。但是 Python 还附带了特殊的`csv`和`json`模块，每个模块都提供了帮助您处理这些文件格式的函数。

CSV 代表“逗号分隔值”，CSV 文件是存储为明文文件的简化电子表格。Python 的`csv`模块使得解析 CSV 文件变得很容易。

JSON(读作“JAY-saw”或“Jason”——怎么读并不重要，因为人们会说你读错了)是一种将信息作为 JavaScript 源代码存储在明文文件中的格式。(JSON 是 JavaScript 对象符号的缩写。)使用 JSON 文件不需要了解 JavaScript 编程语言，但是了解 JSON 格式很有用，因为它在许多 web 应用程序中使用。
...
由于 CSV 文件只是文本文件，您可能会尝试将它们作为字符串读入，然后使用您在第 9 章中学到的技术处理该字符串。例如，由于 CSV 文件中的每个单元格都由逗号分隔，所以您可以在每行文本上调用`split(',')`来获取逗号分隔的值作为字符串列表。但并不是 CSV 文件中的每个逗号都代表两个单元格之间的边界。CSV 文件也有自己的转义字符集，允许逗号和其他字符作为值的一部分包含在*中。`split()`方法不处理这些转义字符。因为这些潜在的陷阱，你应该总是使用`csv`模块来读写 CSV 文件。*
...
在 Windows 上，您还需要为`open()`函数的`newline`关键字参数传递一个空字符串。由于超出本书范围的技术原因，如果你忘记设置`newline`参数，那么 *output.csv* 中的行将是双倍行距，如图[图 16-1](#calibre_link-1566) 所示。
...
该程序将需要用*打开每个文件。csv* 扩展名在当前工作目录下，读入 csv 文件的内容，将没有第一行的内容重写到同名文件中。这将用新的无头内容替换 CSV 文件的旧内容。
...
调用`os.makedirs()`将创建一个`headerRemoved`文件夹，所有的无头 CSV 文件将被写入其中。在`os.listdir('.')`上的一个`for`循环可以让你完成一部分，但是它会遍历工作目录中的所有*文件，所以你需要在循环的开始添加一些代码，跳过不以`.csv`结尾的文件名。当遇到非 CSV 文件时，`continue`语句 ➊ 使`for`循环移动到下一个文件名。*

程序运行时会有一些*和*输出，打印出一条消息，说明程序正在处理哪个 CSV 文件。然后，添加一些关于程序其余部分应该做什么的`TODO`注释。
...
为了测试你的程序，从 https://nostarch.com/automatestuff2/的下载 *removeCsvHeader.zip* 并解压到一个文件夹中。运行该文件夹中的 *removeCsvHeader.py* 程序。输出将如下所示:
...
了解 JSON 是很有用的，因为许多网站提供 JSON 内容作为程序与网站交互的一种方式。这被称为提供一个*应用编程接口(API)* 。访问 API 与通过 URL 访问任何其他网页是一样的。区别在于 API 返回的数据是为机器格式化的(例如用 JSON );API 不容易让人读懂。
...
Python 的`json`模块为`json.loads()`和`json.dumps()`函数处理带有 JSON 数据的字符串和 Python 值之间转换的所有细节。JSON 不能存储每一种 Python 值的*。它只能包含以下数据类型的值:字符串、整数、浮点、布尔、列表、字典和`NoneType`。JSON 不能表示特定于 Python 的对象，比如`File`对象、CSV `reader`或`writer`对象、`Regex`对象或 Selenium `WebElement`对象。*
...
实际上，如果你有一个程序可以下载未来几天的天气预报并以纯文本格式打印出来，那么你可以跳过很多无聊的步骤。这个程序使用第 12 章中[的`requests`模块从网上下载数据。](#calibre_link-372)
...
对于这个项目，打开一个新的文件编辑器窗口，并将其保存为 *getOpenWeather.py* 。然后在你的浏览器中访问 https://openweathermap.org/api/的并注册一个免费帐户，以获得一个 *API 密钥*，也称为应用 ID，对于 OpenWeatherMap 服务来说，它是一个类似于`'30144aba38018987d84710d0e319281e'`的字符串代码。除非你计划每分钟进行 60 次以上的 API 调用，否则你不需要为这项服务付费。对 API 密钥保密；任何知道它的人都可以编写使用您帐户的使用配额的脚本。
...
OpenWeatherMap 服务要求查询格式为城市名、逗号和两个字母的国家代码(如“US”代表美国)。你可以在 https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2 的找到这些代码的列表。我们的脚本显示检索到的 JSON 文本中列出的第一个城市的天气。不幸的是，同名的城市，如俄勒冈州的波特兰和缅因州的波特兰，都将被包括在内，尽管 JSON 文本将包括经度和纬度信息以区分这两个城市。
...
*OpenWeatherMap.org*以 JSON 格式提供实时天气信息。首先你必须在网站上注册一个免费的 API 密匙。(此键用于限制您在他们的服务器上发出请求的频率，以降低他们的带宽成本。)您的程序只需下载位于 https://api.openweathermap.org/data/2.5/forecast/daily?q=*的页面<位置>&CNT = 3&APPID =<APIkey>*，其中*位置<位置>* 是您想要了解其天气的城市名称， *< API key >* 是您的个人 API key。将以下内容添加到 *getOpenWeather.py* 中。
...
您可以通过将`weatherData`传递给`pprint.pprint()`来查看这些数据。你可能想查看 https://openweathermap.org/的以获得更多关于这些字段含义的文档。例如，在线文档会告诉你`'day'`后的`302.29`是白天的开尔文温度，而不是摄氏度或华氏度。
...
*   制定一个计划，定期检查天气，如果你需要将植物移到室内，会给你发送霜冻警告。([第 17 章](#calibre_link-530)讲述日程安排，[第 18 章](#calibre_link-567)解释如何发送电子邮件。)
*   从多个站点获取天气数据并一次显示，或者计算并显示多个天气预测的平均值。

### **总结**

CSV 和 JSON 是存储数据的常见明文格式。它们很容易被程序解析，同时仍然是人类可读的，所以它们通常用于简单的电子表格或 web 应用程序数据。`csv`和`json`模块大大简化了 CSV 和 JSON 文件的读写过程。

前几章已经教你如何使用 Python 来解析各种文件格式的信息。一个常见的任务是从各种格式中提取数据，并对其进行解析以获得您需要的特定信息。这些任务通常特定于商业软件没有最佳帮助的情况。通过编写自己的脚本，您可以让计算机处理以这些格式呈现的大量数据。

在第 18 章中，你将脱离数据格式，学习如何让你的程序通过发送电子邮件和文本信息与你交流。

### **练习题**

[1](#calibre_link-1567) 。Excel 电子表格有哪些 CSV 电子表格没有的功能？

[2](#calibre_link-1568) 。你传递给`csv.reader()`和`csv.writer()`什么来创建`reader`和`writer`对象？

[3](#calibre_link-1569) 。`reader`和`writer`对象的`File`对象需要在什么模式下打开？

[4](#calibre_link-1570) 。什么方法获取列表参数并将其写入 CSV 文件？

[5](#calibre_link-1571) 。`delimiter`和`lineterminator`关键字参数是做什么的？

[6](#calibre_link-1572) 。什么函数接受一串 JSON 数据并返回一个 Python 数据结构？

[7](#calibre_link-1573) 。哪个函数采用 Python 数据结构并返回一串 JSON 数据？
...
Excel 只需点击几下鼠标就可以将电子表格保存为 CSV 文件，但是如果您必须将数百个 Excel 文件转换为 CSV 文件，则需要花费数小时的点击时间。使用第十二章的[中的`openpyxl`模块，编写一个程序，读取当前工作目录中的所有 Excel 文件，并将其输出为 CSV 文件。](#calibre_link-372)

一个 Excel 文件可能包含多个工作表；您必须为每张*表单*创建一个 CSV 文件。CSV 文件的文件名应为 *< excel 文件名> _ <工作表标题>。csv* ，其中 *< excel 文件名>* 是不带文件扩展名的 excel 文件的文件名(例如，`'spam_data'`，而不是`'spam_data.xlsx'`)*<工作表标题>* 是来自`Worksheet`对象的`title`变量的字符串。
...
从【https://nostarch.com/automatestuff2/】的 *[下载 ZIP 文件 *excelSpreadsheets.zip* 并将电子表格解压到与你的程序相同的目录下。您可以将这些文件用作测试程序的文件。](https://nostarch.com/automatestuff2/)*