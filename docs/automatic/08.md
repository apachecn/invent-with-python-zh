...
假设您想将区号与电话号码的其余部分分开。添加括号将在 regex: `(\d\d\d)-(\d\d\d-\d\d\d\d)`中创建*分组*。然后，您可以使用`group()` match 对象方法从一个组中获取匹配的文本。
...
这里，`(Ha){3}`与`'HaHaHa'`匹配，但与`'Ha'`不匹配。由于与`'Ha'`不匹配，`search()`Return`None`。
...
22 。如何编写一个正则表达式来匹配第一个单词是`Alice`、`Bob`或`Carol`的句子？第二个字要么是`eats`、`pets`，要么是`throws`；第三个字是`apples`、`cats`，或者`baseballs`；句子以句号结尾？这个正则表达式应该不区分大小写。它必须符合以下条件:
