...
注意代码是`spam.append('moose')`和`spam.insert(1, 'chicken')`，而不是`spam = spam.append('moose')`和`spam = spam.insert(1, 'chicken')`。`append()`和`insert()`都不会给出`spam`的新值作为其返回值。(事实上，`append()`和`insert()`的返回值是`None`，所以您肯定不想将它存储为新的变量值。)相反，该列表在处被修改*。在第 94 页的上的可变和不可变数据类型中会更详细地介绍就地修改列表。*
