...
Word 和其他文字处理程序使用样式来保持相似类型文本的视觉渲染一致并易于更改。例如，您可能希望将正文段落设置为 11 磅、Times New Roman、左对齐、右不齐的文本。您可以使用这些设置创建样式，并将其分配给所有正文段落。然后，如果您以后想要更改文档中所有正文段落的显示，您只需更改样式，所有这些段落都会自动更新。
