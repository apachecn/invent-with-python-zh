...
在 `restyled.docx` 顶部的单词*文档标题*将具有普通样式而不是标题样式，用于文本*的`Run`对象(带有一些*的普通段落)将具有 QuoteChar 样式，用于单词*粗体* 和*斜体*的两个`Run`对象的`underline`属性将被设置为`True`。图 15-7 显示了 `restyled.docx` 中段落和段落的样式。
...
1 。PDF 文件名的字符串值是传递给`PyPDF2.PdfFileReader()`函数的*而不是*。你传递给函数的是什么呢？
