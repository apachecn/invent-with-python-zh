...
如果你在调试器下运行这个程序，你将不得不在程序终止前点击成千上万次“跳过”按钮。如果你对程序执行到一半时`heads`的值感兴趣，当 1000 次抛硬币中的 500 次完成时，你可以在`print('Halfway done!')` ➋ 行设置一个断点。要设置断点，点击文件编辑器中的行号，出现一个红点，如图 11-4 中[所示标记断点。](#calibre_link-1198)
