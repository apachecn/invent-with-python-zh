...
凭据文件将生成令牌文件。获取凭证文件最简单的方法是在`developers.google.com/sheets/api/quickstart/python`](https://developers.google.com/sheets/api/quickstart/python/)进入 Google Sheets Python 快速入门页面，点击蓝色的**启用 Google Sheets API** 按钮，如图图 14-1 所示。您需要登录您的 Google 帐户才能查看此页面。
