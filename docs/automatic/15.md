...
如上所述，一次写入一个单元的数据通常会花费很长时间。幸运的是，EZSheets 拥有同时读写整列和整行的`Sheet`方法。`getColumn()`、`getRow()`、`updateColumn()`和`updateRow()`方法将分别读写列和行。这些方法向 Google Sheets 服务器请求更新电子表格，因此它们要求您连接到互联网。在本节的例子中，我们将把上一章的*produces sales . xlsx*上传到 Google Sheets。前八行看起来像表 14-1 。

**表 14-1:***生产销售. xlsx* 电子表格的前八行
...
5 。`ss`变量包含一个`Spreadsheet`对象。什么代码将从标题为“学生”的工作表中的单元格 B2 中读取数据？
...
10 。如果通过频繁地使用 EZSheets 进行读写请求，超出了 Google 帐户的限额，会发生什么情况？
