...
这里我们有一个 2019 年 10 月 21 日下午`4:29`的`datetime`对象，存储在`oct21st`中。传递自定义格式字符串`'%Y/%m/%d %H:%M:%S'`的`strftime()`会返回一个字符串，其中包含由斜杠分隔的 2019、10 和 21 以及由冒号分隔的 16、29 和 00。通过`'%I:%M% p'`Return`'04:29 PM'`，通过`"%B of '%y"`Return`"October of '19"`。注意`strftime()`不是以`datetime.datetime`开头的。
...
这里我们开启一个 MS 喷漆流程 ➊ 。当它还在运行时，我们检查`poll()`是否返回`None` ➋ 。应该是，因为该进程仍在运行。然后我们关闭 MS Paint 程序，并在终止的进程 ➌ 上调用`wait()`。现在`wait()`和`poll()`Return`0`，表示过程结束，没有错误。
