...
导入我们需要的模块后，我们在 ➊ 做一个目录存放漫画，开始定义`downloadxkcd()`➋。我们遍历指定范围内的所有数字 ➌ 并下载每个页面 ➍。我们用 BeautifulSoup 浏览每一页的 HTML ➎ 找到漫画形象 ➏。如果在页面上找不到漫画图像，我们就打印一条消息。否则，我们获取图片 ➐ 的 URL 并下载图片 ➑。最后，我们将图像保存到我们创建的目录中。
