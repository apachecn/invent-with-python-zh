...
PyInputPlus 不是 Python 标准库的一部分，所以必须使用 Pip 单独安装。要安装 PyInputPlus，请从命令行运行`pip install --user pyinputplus`。[附录 A](#calibre_link-2) 有安装第三方模块的完整说明。要检查 PyInputPlus 是否安装正确，请在交互式 shell 中导入它:
...
使用 Python 的`help()`函数来了解关于这些函数的更多信息。例如，`help(pyip.inputChoice)`显示`inputChoice()`功能的帮助信息。完整的文档可以在 https://pyinputplus.readthedocs.io/[找到。](https://pyinputplus.readthedocs.io/)
...
PyInputPlus 模块的函数可以让您不必自己编写繁琐的输入验证代码。但是 PyInputPlus 模块有比这里详细描述的更多的内容。你可以在 https://pyinputplus.readthedocs.io/的在线查看它的完整文档。
...
如果 PyInputPlus 模块的函数都不符合您的需要，但是您仍然喜欢 PyInputPlus 提供的其他特性，您可以调用`inputCustom()`并传递您自己的自定义验证函数供 PyInputPlus 使用。https://pyinputplus.readthedocs.io/en/latest/T2*的文档中有 PyInputPlus 函数和附加特性的完整列表。PyInputPlus 在线文档中的内容比本章中描述的要多得多。重新发明轮子是没有用的，学会使用这个模块将使你不必自己编写和调试代码。*

现在您已经掌握了处理和验证文本的专业知识，是时候学习如何读写计算机硬盘上的文件了。

### **练习题**

[1](#calibre_link-1098) 。PyInputPlus 是 Python 标准库自带的吗？

[2](#calibre_link-1099) 。为什么 PyInputPlus 一般用`import pyinputplus as pyip`导入？

[3](#calibre_link-1100) 。`inputInt()`和`inputFloat()`有什么区别？

[4](#calibre_link-1101) 。如何确保用户使用 PyInputPlus 输入一个介于`0`和`99`之间的整数？

[5](#calibre_link-1102) 。传递给`allowRegexes`和`blockRegexes`关键字参数的是什么？

[6](#calibre_link-1103) 。空白输入三次`inputStr(limit=3)`做什么？

[7](#calibre_link-1104) 。空白输入三次`inputStr(limit=3, default='hello')`做什么？
...
将您的代码与第 196 页[的“](#calibre_link-1047)[项目:乘法测验](#calibre_link-284)”中使用 PyInputPlus 的代码进行比较。