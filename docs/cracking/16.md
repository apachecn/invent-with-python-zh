...
选择**文件 -> 新文件**，打开新文件编辑器窗口。在文件编辑器中输入以下代码，然后保存为 `affineHacker.py` 。手动输入`myMessage`变量的字符串可能有些棘手，所以你可以从[`www.nostarch.com/crackingcodes`](https://www.nostarch.com/crackingcodes/)的 `affineHacker.py` 文件中复制并粘贴它以节省时间。确保 `dictionary.txt` 以及 `pyperclip.py` 、`affinicipher.py`、 `detectEnglish.py` 和 `cryptomath.py` 与*affini hacker . py*在同一个目录下。
