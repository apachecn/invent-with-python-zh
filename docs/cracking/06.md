...
我们在[第四章](#calibre_link-57)中制作的反向密码总是以同样的方式加密。但是凯撒密码使用密钥，根据使用的密钥不同，加密信息的方式也不同。凯撒密码的密钥是从`0`到`25`的整数。即使密码分析人员知道使用了凯撒密码，这也不足以给他们充分的信息来破解密码，除非他们知道对应的密钥。
...
在文件编辑器中输入以下代码，并保存为 *caesarCipher.py* 。然后从[T5【https://www.nostarch.com/crackingcodes/】T6](https://www.nostarch.com/crackingcodes/)下载 *pyperclip.py* 模块，放在与文件 *caesarCipher.py* 相同的目录(即同一个文件夹)下。该模块将被 *caesarCipher.py* 导入；我们将在第 56 页的[的](#calibre_link-333)[导入模块和设置变量](#calibre_link-333)中对此进行更详细的讨论。
...
方法是附加到特定数据类型的值的函数。`find()` string 方法返回传递给它的字符串参数在被调用的字符串中的位置的整数值。
