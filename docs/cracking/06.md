...
在文件编辑器中输入以下代码，并保存为 `caesarCipher.py` 。然后从T5`www.nostarch.com/crackingcodes`T6](https://www.nostarch.com/crackingcodes/)下载 `pyperclip.py` 模块，放在与文件 `caesarCipher.py` 相同的目录(即同一个文件夹)下。该模块将被 `caesarCipher.py` 导入；我们将在第 56 页的[的导入模块和设置变量中对此进行更详细的讨论。
