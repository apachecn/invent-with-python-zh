...
使用单词模式，我们可以猜测密文可能解密成哪些明文字母，我们称之为密文的*潜在解密字母*。要破解用简单替换密码加密的消息，我们需要找到消息中每个单词的所有潜在解密字母，并通过排除过程确定实际的解密字母。[表 17-1](#calibre_link-384) 列出了 HGHHU 的潜在解密字母。
...
以下是使用[表 17-1](#calibre_link-384) 创建的*密码字母映射*:
...
为了用 Python 代码表示[表 17-1](#calibre_link-384) ，我们将使用一个字典值来表示密码字母映射，如下所示(`'H'`、`'G'`和`'U'`的键值对以粗体显示):
...
要计算 *dictionary.txt* 字典文件中每个单词的单词模式，从【https://www.nostarch.com/crackingcodes/】的[T5T7】下载*makewodpatterns . py*。确保这个程序和 *dictionary.txt* 都在保存本章的 *simpleSubHacker.py* 程序的文件夹中。](https://www.nostarch.com/crackingcodes/)
...
`re.compile()`函数创建一个`re`模块可以使用的正则表达式模式对象(缩写为 *regex 对象*或*模式对象*)。我们将使用这个对象从第 241 页的[的](#calibre_link-385)[`hackSimpleSub()`函数](#calibre_link-385)中删除任何非字母字符。

您可以使用正则表达式执行许多复杂的字符串操作。要了解更多关于正则表达式的信息，请转到[*https://www.nostarch.com/crackingcodes/*](https://www.nostarch.com/crackingcodes/)。
...
`hackSimpleSub()`不是返回解密的消息或`None`(如果无法解密的话),而是返回一个删除了解密字母的相交密码字母映射。(我们将在第 234 页的[上的](#calibre_link-386)[相交两个映射](#calibre_link-386)中查看如何创建相交映射。)这个相交的密文映射然后被传递给`decryptWithCipherletterMapping()`以将存储在`message`中的密文解密成可读格式，你将在第 243 页的[中的“](#calibre_link-387)[解密消息](#calibre_link-387)中看到更多细节。
...
当我们从密文中得到几个单词的字母映射时，我们将使用一个函数将它们合并在一起。然后，我们将使用最后一个辅助函数，通过将一个解密字母与每个密码字母匹配来解决尽可能多的密码字母的解密。如上所述，我们不可能总是能够解开所有的密码，但是你会在第 243 页的[的“](#calibre_link-387)[解密信息](#calibre_link-387)中找到如何解决这个问题。
...
让我们通过在 inter active shell 中测试来看看`removeSolvedLetterFromMapping()`的运行情况。返回到创建`intersectedMapping`时打开的交互式 shell 窗口。(如果你关了窗户，不用担心；你可以重新输入第 235 页的[的“](#calibre_link-388)[字母映射帮助函数如何工作](#calibre_link-388)中的指令，然后跟着这个例子做。)
...
让我们回到我们在第 235 页的[的“](#calibre_link-388)[字母映射帮助函数如何工作](#calibre_link-388)”中使用的例子。我们将使用我们在前面的 shell 示例中创建的`intersectedMapping`变量来解密密文消息`'OLQIHXIRCKGNZ PLQRZKBZB MPBKSSIPLC'`。
...
简单替换密码的主要优点是它有大量可能的密钥。缺点是比较密码和字典文件中的单词来确定哪个密码解密成哪个字母相对容易。在第 18 章中，我们将探索一种更强大的多字母替代密码，称为 Vigenère 密码，这种密码几百年来都被认为是无法破解的。
